...
def peak_mask(audio):
    ...
    peaks, _ = find_peaks(audio, height=0.5)
    mask[peaks] = True
    
    return audio[~mask]

# plt.figure(figsize=(14, 6))
# plt.plot(y, label = 'raw audio', color = 'r')

# y = peak_mask(y)
# plt.plot(y, label = 'masking', color = 'skyblue')
...
for i, audio_path in enumerate(df_test_label['path']):    
    #진행 상황
    if i % (len(df_test_label) // 1000) == 0: print(f'{i/len(df_test_label)*100}%')
